...
M.sources = {
  { name = "utils", desc = "LazyVim extras", module = "lazyvim.plugins.extras" },
...
function M.get_extra(source, modname)
  utils.plugin.handle_defaults = false
...
function X.new()
  ...
  self.float = Float.new({ title = "utils Extras" })
    ...
    if extra.row == pos[1] then
      ...
      utils.info(
        "`"
          ...
          .. "\nPlease restart utils to apply the changes.",
        { title = "LazyExtras" }
      )
      self:update()
      return
...
function X:render()
  self.text:nl():nl():append("utils Extras", "LazyH1"):nl():nl()
  self.text
    :append("This is a list of all enabled/disabled utils extras.", "LazyComment")
    :nl()
...
function X:extra(extra)
  local defaults = utils.config.get_defaults()
  ...
  if not extra.managed then
    ...
    elseif vim.tbl_contains(utils.plugin.core_imports, extra.module) or origin == "default" then
      self:diagnostic({
  ...
  if extra.source.name ~= "utils" then
